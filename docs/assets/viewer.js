...
const HAIR_OVERRIDES = {
  sander_head__3_: {
    global: {
      hair_roughness_floor:    0.00,
      root_darkening:          0.00,
      seed_variation:          0.36,
      hair_roughness_seed_amp: 0.10,
      anisotropy:              0.78,
      anisotropy_rotation:     0.028,
      hair_view_ao:            0.68,
    },
    materials: {
      hair: { mode: 'both', color: '#907664', hair_density: 1.0, blend_opacity: 1.0, spec: 0.21 },
    },
  },
  taro: {
    global: {
...
export async function mount(container, opts) {
  ...
  // The IBL alone is flat (no form, no shadow). Drop it to a soft ambient and
  ...
  rimLight.position.set(0.5, 0.9, -1.6);    // back-top rim to separate hair
  scene.add(rimLight);
  ...
  // Attach the live AO control to EVERY material that actually carries the
  // baked COLOR_0 occlusion (the face-skin slots). The skin isn't always in
  // mh_materials.json (facescan exports often only map hair), so this runs
  // off the actual vertex-color flag, not the mapping. Without this the AO
  // would render at full strength via three.js's default vColor multiply,
  // with no panel control — which is exactly the "too dark, no sliders" case.
  gltf.scene.traverse((o) => {
    if (!o.isMesh) return;
    const mats = Array.isArray(o.material) ? o.material : [o.material];
    for (const m of mats) {
      if (m && m.vertexColors && !m.userData?.hairPass) {
        injectAOControl(m);
        if ('specularIntensity' in m) m.specularIntensity = 0.99;  // operator-tuned skin spec
        if (!skinMats.includes(m)) skinMats.push(m);
      }
    }
  });
  console.log('[viewer] AO control on', skinMats.length, 'vertex-color material(s)');
  ...
  if (tuneOn && (hairMats.length > 0 || skinMats.length > 0)) {
    // Force material compilation NOW so every onBeforeCompile has run and the
    // hair/skin shader uniforms (mat.userData.hairUniforms / skinUniforms)
    // exist before the panel reads them. Without this the sliders initialise
    // from fallbacks (e.g. view AO shows 0.0) instead of the applied override
    // values (0.68) — the "view AO isn't being set" symptom.
...
// Live shaping of the baked COLOR_0 occlusion (no re-bake). three.js multiplies
// vColor into diffuse in <color_fragment>; reshape it so the panel can tame it:
//   uAOFloor    lifts the darkest (deepest-occluded) areas toward white — the
//               "by area" control, since scalp/neck/cavity regions are the ones
//               crushed near black on export;
//   uAOContrast gamma-reshapes the falloff;
//   uAOStrength overall amount (0 = no AO, 1 = full).
// Applied to every material that actually carries vertex colors (the baked AO),
// regardless of whether it appeared in the material mapping.
function injectAOControl(mat) {
  if (!mat || mat.userData?._aoApplied) return;
  mat.userData = mat.userData || {};
  mat.userData._aoApplied = true;
  uniqueCacheKey(mat);
  const prevOBC = mat.onBeforeCompile;
  mat.onBeforeCompile = (shader) => {
    if (prevOBC) prevOBC(shader);
    ...
    shader.fragmentShader =
      ...
      + shader.fragmentShader;
    // COLOR_0.R = broad Cycles AO, COLOR_0.G = root-contact AO (hair-root/scalp
    // interface). Drive them with independent sliders.
    shader.fragmentShader = shader.fragmentShader.replace(
      '#include <color_fragment>',
      `
      #if defined( USE_COLOR_ALPHA )
        vec3 _vc = clamp( vColor.rgb, 0.0, 1.0 );
        diffuseColor.a *= vColor.a;
      #elif defined( USE_COLOR )
        vec3 _vc = clamp( vColor, 0.0, 1.0 );
      #endif
        ...
        // through tonemapping instead of darkening.
        _broad = clamp( mix( 1.0, _broad, uAOStrength ), 0.0, 1.0 );   // broad amount
        _root  = clamp( mix( 1.0, _root, uRootAOStrength ), 0.0, 1.0 ); // root amount
        diffuseColor.rgb *= _broad * _root;
      #endif
      `
    );
  };
...
function patchMaterials(root, mapping, baseUrl) {
  ...
  const skinMats = [];   // collected for the live AO / spec panel
  ...
  // Hair does NOT cast runtime shadows: even at 4096 + soft PCF the thin
  // strands alias into blocky blobs in the shadow map. The soft hair-on-skin
  // shadow comes from a BAKED AO map (stage 02) instead.
  for (const { mesh } of hairTwoPass) {
  ...
  for (const { mesh, outerMat } of hairTwoPass) {
    if (outerMat.alphaToCoverage) {
      ...
      // DoubleSide + transparent renders single-pass by default, so the back
      // faces aren't sorted against the front faces — at grazing angles the
      // back half of each card draws wrong and shows as a sliver. Force the
      // proper two-sub-pass render (back faces, then front faces).
      blendMat.forceSinglePass = false;
      // alphaTest is set below, AFTER opacity — it must scale with opacity.
      ...
      // Now that the coverage injection actually reaches the blend pass, the
      // soft coverage*density already drives gap alpha to ~0 — no hard cutoff
      // needed. A high alphaTest here snaps card EDGES into visible slivers at
      // grazing angles. Keep it just above zero to drop truly-empty texels.
      blendMat.alphaTest = 0.02;
      uniqueCacheKey(blendMat);
      blendMat.userData = { ...blendMat.userData, hairPass: 'blend' };
      // Material.clone() does NOT copy onBeforeCompile, so blendMat's is the
      // default no-op and the hair coverage injection never reaches the blend
      // pass (it renders the flat card quad -> faint rectangles over skin in
      // "both" mode). Grab the OUTER material's real injection and wrap it to
      // redirect the hairUniforms write to this clone.
      const _origCompile = outerMat.onBeforeCompile;
      if (_origCompile) {
...
function applyHair(mat, p, t, loadTex) {
  ...
  // compact atlases, A=legacy) — NOT albedo. Capture it first: for embedded
  // exports this IS the packed atlas. Then drop it from the colour slot.
  const embeddedAtlas = mat.map || null;
  if (embeddedAtlas) embeddedAtlas.colorSpace = THREE.NoColorSpace;   // raw data
  ...
  if (typeof p.roughness === 'number') mat.roughness = p.roughness;
  // Specular intensity — the white sheen on the hair. The new lights make the
  // shiny hair's spec blow out; expose it as a control (panel + saved JSON).
  const _spec = (typeof matPerOverride?.spec === 'number') ? matPerOverride.spec
              : (typeof p.spec === 'number') ? p.spec : 0.5;
  if ('specularIntensity' in mat) mat.specularIntensity = _spec;
  mat.userData._hairSpecBase = _spec;

  // Atlas source: prefer the GLB-EMBEDDED packed atlas (self-contained — the
  // texture lives in the GLB, no sidecar). Fall back to the legacy sidecar
  // (alpha_stem / textures dict) for older exports.
  let atlas = (p.embedded_atlas && embeddedAtlas) ? embeddedAtlas : null;
  let atlasUrl = atlas ? '(embedded in GLB)' : null;
  if (atlas) {
    // Make the GLB-embedded texture behave IDENTICALLY to a sidecar atlas so
    // the existing (correct) hair shader masks it the same way:
    //   - raw data, not sRGB colour;
    //   - flipY=true: TextureLoader sidecars are flipped; the atlas was
    //     authored for that orientation, but GLTFLoader imports flipY=false;
    //   - force a re-upload + material recompile so USE_ALPHAMAP turns on
    //     (otherwise the alpha mask never samples and cards render opaque).
    atlas.colorSpace = THREE.NoColorSpace;
    atlas.needsUpdate = true;
    mat.needsUpdate = true;
  } else {
    atlasUrl = p.alpha_stem ? `textures/${p.alpha_stem}.png`
             : (t.alpha_r || t.alpha || null);
    atlas = atlasUrl ? loadTex(atlasUrl, { srgb: false }) : null;
  }
  if (!atlas) {
  ...
  // Keep the alpha cut-out sharp at grazing angles. When a card tilts toward
  // edge-on, the atlas minifies to coarse mips that bleed strands into the
  // gaps, so the invisible (cut-out) parts of the card appear as you rotate.
  // Max anisotropic filtering preserves the mask under that minification.
  atlas.anisotropy = 16;
  atlas.needsUpdate = true;
  ...
  // Small bias to win the depth test against the skin where cards sit flush,
  // WITHOUT pulling back-of-head strands in front of the head (-16 units did
  // that — strands showed through the face). Keep it just enough to avoid
  // hairline z-fighting.
  ...
  if ('anisotropy' in mat) {
    if (tangent) mat.anisotropyMap = tangent;
  ...
  // View-dependent hair self-shadow. OFF by default (0): any value lets spec
  // dominate edge-on cards and glow. Opt in per-character via hair_view_ao.
  const viewAO = typeof p.hair_view_ao === 'number' ? p.hair_view_ao : 0.0;
  // Matte the roots (kill spec near the scalp transition). 0 = off, 1 = fully matte.
  const rootSpecKill = typeof p.hair_root_spec_kill === 'number' ? p.hair_root_spec_kill : 0.7;
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.uniforms.uHairViewAO    = { value: viewAO };
    shader.uniforms.uHairRootSpecKill = { value: rootSpecKill };
    ...
    shader.fragmentShader = `
      ...
      uniform float uHairViewAO;
      uniform float uHairRootSpecKill;
    ` + shader.fragmentShader;
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      #include <map_fragment>
        ...
        // View-dependent self-shadow. It darkens ONLY diffuse, so any value
        // > 0 lets specular dominate edge-on/down-pointing cards and makes
        // them "glow" + shift as the camera moves. uHairViewAO = 0 disables
        // it entirely (default); dial it in from the panel only if wanted.
        float ndv = abs( dot( normalize(vNormal), normalize(vViewPosition) ) );
        float hairAO = mix( 1.0 - uHairViewAO, 1.0, smoothstep( 0.05, 0.55, ndv ) );
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      float roughnessFactor = roughness;
        ...
        // Matte the roots so the hair->scalp transition doesn't catch a hot
        // specular highlight (push roughness toward 1 near the root, rootT~0).
        float rootMatte = ( 1.0 - smoothstep( 0.0, 0.35, hairRootT ) ) * uHairRootSpecKill;
        roughnessFactor = mix( roughnessFactor, 1.0, clamp( rootMatte, 0.0, 1.0 ) );
      #endif
...
function buildHairTunePanel(container, hairMats, ctx = {}) {
  ...
  header.addEventListener('click', () => {
    ...
    header.textContent = open ? 'tune ▴' : 'tune ▾';
  });
  ...
  // --- lighting section: studio key/fill/rim + ambient + key direction ---

  const L = ctx.lights || {};
  const T = ctx.THREE;
  if (L.key || L.fill || L.rim || ctx.scene) {
    addSectionHeader(body, 'lighting');
    if (L.key)
      addSlider(body, 'key int', 0.0, 6.0, 0.05, L.key.intensity,
                (v) => { L.key.intensity = v; });
    if (L.fill)
      addSlider(body, 'fill int', 0.0, 3.0, 0.05, L.fill.intensity,
                (v) => { L.fill.intensity = v; });
    if (L.rim)   // the "backlight" that makes down/edge hair glow — turn to 0 to kill it
      addSlider(body, 'rim int', 0.0, 3.0, 0.05, L.rim.intensity,
                (v) => { L.rim.intensity = v; });
    if (ctx.scene && 'environmentIntensity' in ctx.scene)
      addSlider(body, 'ambient', 0.0, 2.0, 0.02, ctx.scene.environmentIntensity ?? 1.0,
                (v) => { ctx.scene.environmentIntensity = v; });
    if (L.key && T && L.key.target) {
      const sph = new T.Spherical().setFromVector3(
        L.key.position.clone().sub(L.key.target.position));
      const applyKey = () => {
        L.key.position.copy(
          new T.Vector3().setFromSpherical(sph).add(L.key.target.position));
      };
      addSlider(body, 'key azim', -3.1416, 3.1416, 0.01, sph.theta,
                (v) => { sph.theta = v; applyKey(); });
      addSlider(body, 'key elev', 0.05, 3.0916, 0.01, sph.phi,
                (v) => { sph.phi = v; applyKey(); });
    }
  }
  ...
  if (skinMats.length) {
    addSectionHeader(body, 'skin');
    const setSkinUniform = (name, v) => {
      for (const m of skinMats) {
        const u = m.userData?.skinUniforms?.[name];
        if (u) u.value = v;
      }
    };
    ...
    addSlider(body, 'skin spec', 0.0, 1.5, 0.01,
              ...
              (v) => { for (const m of skinMats)
                         if ('specularIntensity' in m) m.specularIntensity = v; });
  }
  ...
  // View-dependent hair self-shadow. 0 = off (no spec-dominance glow on
  // edge-on / down-pointing strands); raise only if you want volume darkening.
  addSlider(body, 'view AO',     0.0, 1.0, 0.01, u0.uHairViewAO?.value ?? 0.0,
            (v) => setUniformAll('uHairViewAO', v));
  // Matte the roots near the scalp so the hair->scalp transition stops catching
  // a specular hotspot. 0 = off, 1 = fully matte roots.
  addSlider(body, 'root spec kill', 0.0, 1.0, 0.01, u0.uHairRootSpecKill?.value ?? 0.7,
            (v) => setUniformAll('uHairRootSpecKill', v));
  ...
  for (const [label, mats] of groups) {
    ...
    addSlider(body, 'spec', 0.0, 1.0, 0.01, primary[0]?.specularIntensity ?? 0.5,
              (v) => { for (const m of [...primary, ...blendClones])
                         if ('specularIntensity' in m) m.specularIntensity = v; });
    if (hasClones) {
  ...
  saveBtn.addEventListener('click', () => {
    ...
    out.global.hair_view_ao            = +(u.uHairViewAO?.value ?? 0).toFixed(3);
    out.global.hair_root_spec_kill     = +(u.uHairRootSpecKill?.value ?? 0.7).toFixed(3);

    // Lighting + skin are viewer-global (not per-character hair overrides).
    // Captured here so the chosen rig can be baked into the viewer defaults.
    out._lighting = {
      key_int:  +(L.key?.intensity  ?? 0).toFixed(2),
      fill_int: +(L.fill?.intensity ?? 0).toFixed(2),
      rim_int:  +(L.rim?.intensity  ?? 0).toFixed(2),
      ambient:  +(ctx.scene?.environmentIntensity ?? 0).toFixed(2),
    };
    ...
    for (const [label, mats] of groups) {
      ...
      entry.spec = +(p0?.specularIntensity ?? 0.5).toFixed(2);
      if (hasBlendClone) {
